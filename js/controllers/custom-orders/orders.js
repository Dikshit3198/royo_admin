Royo.controller('CustomOrderManagerCtrl', ['$scope', '$location', 'services', 'factories', '$stateParams', 'pagerService', '$rootScope', 'API', 'constants', '$translate', '$state', '$interval',
  function ($scope, $location, services, factories, $stateParams, pagerService, $rootScope, API, constants, $translate, $state, $interval) {
    ...
    $scope.getOrderData = function (page) {
      ...
      param_data.status = $scope.tabStatus;
      ...
      services.GET_DATA(param_data, API.GET_CUSTOM_ORDER_LIST, function (response) {
        $scope.count = response.data.count;
        $scope.orders = response.data.list ? response.data.list : [];
    ...
    $scope.updateOrderStatus = function (val, orderId, self_pickup) {
      ...
      param_data.custom_order_id = orderId;
      param_data.status = val;
      // param_data.offset = offset;

      

      services.POST_DATA(param_data, API.CONFIRM_REJECT_CUSTOM_ORDER, function (response) {
        afterStatusUpdate();
      });
    ...
    $scope.orderConfirmation = function (status, order, add_prep_time) {
      ...
      // param_data.accessToken = constants.ACCESSTOKEN;
      // param_data.authSectionId = 36;
      param_data.custom_order_id = order.id;
    ...
    $scope.listAgents = function (order) {
      ...
      if ($rootScope.profile == 'SHIPPING') {
        ...
        param_data.supplierId = 0; // order.supplier_id ? order.supplier_id : 0;
      }
    ...
    $scope.assignAgent = function () {
      ...
      data.customOrderId = $scope.selected_order.id;
      data.sectionId = 36;
      services.POST_DATA(data, API.ASSIGN_AGENT_CUSTOM_ORDER, function (response) {
        $scope.getOrderData();
    ...
    $scope.viewRejectionReason = function(order) {
      $scope.rejection_reason = order.reason; // approve_rejection_reason;
    }
